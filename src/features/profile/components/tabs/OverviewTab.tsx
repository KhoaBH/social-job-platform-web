...
import { Link2, Users, BarChart2, Search, Eye, FileText, GraduationCap, ArrowRight } from "lucide-react";
...
export default function OverviewTab({ onTabChange }: OverviewTabProps) {
  ...
  const analyticsItems = [
    ...
    { icon: <BarChart2 size={20} />, num: u.postImpressions, label: "Lượt hiển thị bài" },
    { icon: <Search size={20} />, num: u.searchAppearances, label: "Xuất hiện tìm kiếm" },
  ];
      ...
      <div className="grid grid-cols-[1fr_300px] gap-4 max-[900px]:grid-cols-1">
        ...
        <div className="flex flex-col gap-3">
          ...
          <section className="bg-white rounded-2xl p-5 shadow-sm">
            ...
            <p className="text-[14px] text-gray-600 leading-relaxed whitespace-pre-line">{u.bio}</p>
              ...
              <div className="px-4.5 pb-3.5 flex flex-col gap-3">
                {analyticsItems.map((item) => (
                  <div key={item.label} className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-[15px] font-bold text-gray-900">{item.num}</div>
                      <div className="text-[12px] text-gray-400">{item.label}</div>
                    </div>
                  </div>
                ))}
              </div>
              <button className="w-full flex items-center justify-center gap-1 py-3 border-t border-gray-100
          ...
          {u.isOwner && (
            <div className="bg-white rounded-2xl overflow-hidden shadow-sm">
              ...
              <div className="px-4.5 pb-4 flex gap-3 items-start">
                ...
                <div>
                  <div className="text-[13.5px] font-semibold text-gray-900">Thêm tóm tắt hồ sơ</div>
                  <div className="text-[12px] text-gray-500 mt-0.5 leading-relaxed">
                    Hồ sơ có tóm tắt nhận được nhiều lượt xem hơn 3.9 lần
                  </div>
                  <button className="mt-2 px-4 py-1.5 border-[1.5px] border-violet-600 rounded-full text-violet-600
